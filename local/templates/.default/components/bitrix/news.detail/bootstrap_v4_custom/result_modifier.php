...
if ($arParams["DISPLAY_BOTTOM_PAGER"] == "Y") {
    $objSeconds = CIBlockElement::GetPropertyValues($arResult["IBLOCK_ID"], ['ID' => $arResult["ID"]], true, ['ID' => 25]);
    if ($arSeconds = $objSeconds->Fetch())
    {
        $seconds = $arSeconds[25];
    }
    $items_prev = CIBlockElement::GetList(
        ["ACTIVE_FROM" => "DESC"],
        ["IBLOCK_ID" => $arResult["IBLOCK_ID"], "ACTIVE" => "Y", "<PROPERTY_SECONDS_ACTIVE" => $arSeconds[25]],
        false,
        ["nTopCount" => 1],
        ["DETAIL_PAGE_URL"]
    );
    $arResult['PREV_POST'] = $items_prev->GetNext()["DETAIL_PAGE_URL"];
    ...
    $arResult['NEXT_POST'] = $items_next->GetNext()["DETAIL_PAGE_URL"];
}